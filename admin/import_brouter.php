...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'preview') {
        ...
        $result = parseBRouterCSV($_FILES['csv_file']['tmp_name']);
...
/**
 * Parsea un CSV de BRouter y devuelve coordenadas + GeoJSON.
 *
 ...
 */
function parseBRouterCSV(string $filePath): array
{
    ...
    while (($row = fgetcsv($handle, 4096, $separator)) !== false) {
        if ($header === null) {
            ...
            // Verificar columnas mínimas del formato BRouter
        ...
        $data = parseBRouterRow($header, $row);
        if ($data) {
    ...
    // Tipo de vía predominante (rail / subway / tram / light_rail / ...)
    $railTypeCounts = array_count_values($railTypes);
    arsort($railTypeCounts);
    $dominantRail = array_key_first($railTypeCounts) ?? 'unknown';

    // GeoJSON con estructura idéntica a las rutas existentes en la BD
    // properties es objeto {}, no array [] (el sistema espera ese formato)
    $geojsonData = json_encode([
        ...
        'properties' => [
            ...
            // 'name' y 'date' se agregan en el paso de confirmación
        ],
        'geometry' => [
            'type'        => 'LineString',
    ...
    // Validar rango int UNSIGNED ya en el parseo
    $distMeters = max(0, min($totalDistanceM, MAX_DIST_METERS));

    return [
        'success'         => true,
        'coordinates'     => downsampleCoordinates($coordinates, 500), // muestra reducida para Leaflet
        'waypoints_count' => count($coordinates),
        'distance_km'     => round($totalDistanceM / 1000, 2),
        'distance_meters' => $distMeters,
        'duration_min'    => round($totalTimeSec / 60),
        'rail_type'       => $dominantRail,
        'geojson_data'    => $geojsonData,  // va a sesión, NO al cliente
...
function parseBRouterRow(array $header, array $row): ?array
{
...
function downsampleCoordinates(array $coords, int $maxPoints): array
{
    $total = count($coords);
...
function getBoundingBox(array $coords): array
{
